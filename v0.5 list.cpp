...
void Skaitymas(std::string z, int &y, int &p, int &c, int &b, std::list<Studentas> &Stud, std::list<Studentas>::iterator it)
{
    std::ifstream fd (z);
    p*=10;
    c=b;
    ...
    for (int i=c ; i<b; i++)
    {
        ...
        fd >> (it)->vardas >> (it)->pav >> (it)->gal;
        std::advance(it, 1);
    }
    y--;

    if (y==4)
        Skaitymas("Antras.txt", y, p, c, b, Stud, it);

    else if (y==3)
        Skaitymas("Trecias.txt", y, p, c, b, Stud, it);

    else if (y==2)
        Skaitymas("Ketvirtas.txt", y, p, c, b, Stud, it);

    else if (y==1)
        Skaitymas("Penktas.txt", y, p, c, b, Stud, it);
}
...
int main()
{
    ...
    else
    {
        ...
        y=5;
        p=100;
        std::list<Studentas>::iterator it = Stud.begin();
        auto start2 = std::chrono::high_resolution_clock::now();
        Skaitymas("Pirmas.txt", y, p, c, b, Stud, it);
        ...
        std::list<Studentas>::iterator it1 = Stud.begin();
        for (int i=0; i<b; i++)
        {
            if ((it1)->gal>=5.0)
            {
                for (int j=i; j<b; j++,i++)
                {
                    StudK+=(it1)->vardas+" "+(it1)->pav+" "+std::to_string((it1)->gal)+"\n";
                    std::advance(it1, 1);
                }

            }
            StudV+=(it1)->vardas+" "+(it1)->pav+" "+std::to_string((it1)->gal)+"\n";
            std::advance(it1, 1);
        }
